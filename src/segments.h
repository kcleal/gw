    ...
    while (true) {
        ...
        while (true) {   // consume md tag until deletion reached
            if (! md_block.is_mm) {  // deletion or end or md tag
                ...
                while (! ins_q.empty() && s_idx + md_block.matches >= ins_q[0].c_s_idx) {  // catch up with insertions
                    ins_q.pop_front();
                }
...
//    } else {
//
//        std::vector<thread> threads_vec(n);
//        int start = 0;
//        int finish = (*aligns).size();
//        int thread_i = 0;
//        int step = finish / n;
//
//        for (thread_i = 0; thread_i < n - 1; i++) {
//            threads_vec[thread_i] = thread(doSomething, i + 1);
//        }
//
//        for (auto& th : threads) {
//            th.join();
