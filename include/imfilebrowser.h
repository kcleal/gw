...
namespace ImGui
{
    ...
    public:
        ...
        // set the label for the ok/confirm button (default: "ok")
        void SetOkButtonLabel(const std::string& label) { okButtonLabel_ = label.empty() ? "ok" : label; }

        // peek at currently selected filenames without consuming the selection
        // (i.e. isOk_ is not changed; safe to call before HasSelected() returns true)
        std::vector<std::filesystem::path> PeekSelected() const
        {
            std::vector<std::filesystem::path> result;
            result.reserve(selectedFilenames_.size());
            for(const auto& name : selectedFilenames_)
                result.push_back(currentDirectory_ / name);
            return result;
        }
    ...
    const FileBrowser &copyFrom)
{
    ...
    statusStr_      = "";
    okButtonLabel_  = copyFrom.okButtonLabel_;
...
inline void ImGui::FileBrowser::Display()
{
    ...
    if(!(flags_ & ImGuiFileBrowserFlags_SelectDirectory))
    {
        ...
        const bool ok = Button(okButtonLabel_.c_str());
        EndDisabled();
    ...
    {
        if(Button((" " + okButtonLabel_ + " ").c_str()) || isEnterPressed)
        {
            isOk_ = true;
            CloseCurrentPopup();
        }
